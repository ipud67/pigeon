// app/story/[id]/page.tsx — every headline IS its own long form (spec §B).
//
// Section order:
...
//
// Depth fields come from data/depth-overrides.json (Research's research) when present, else the
...
// before render (Research editorial must-fix).
...
export default async function Story({ params }: { params: Promise<{ id: string }> }) {
 ...
 {/* FULL EVENT — a labeled linkout to the whole event, start to finish. Not
 buried in the source list. Linkout only — never embedded or hosted (). */}
 {fe ? (
 <section className="full-event" data-testid="full-event">
 <span className="fe-label">Full event</span>
 <a href={fe.url} target="_blank" rel="noopener noreferrer" className="fe-link">
 {fe.label}, {fmtDate(fact.datetime_utc)} · {fe.outlet} <span className="fe-arrow">→</span>
 </a>
 ...
 {fact.longform_url && fact.longform_url !== fe?.url ? (
