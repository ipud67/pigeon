...
export function Masthead({ count, activeNav }: { count?: number; activeNav?: string }) {
 ...
 <Link href="/events" className={activeNav === 'events' ? '' : 'muted'}>
 Full Events
 </Link>
 <Link href="/local" className={activeNav === 'local' ? '' : 'muted'}>
 Local
 </Link>
 </nav>
 <ThemeToggle />
...
// "facts, not takes · every claim links to its primary source" was the same self-narrating copy
// struck from the pages, and it sat on EVERY page. The Creatus LLC line stays — that one is a
// licensing requirement, not a description.
