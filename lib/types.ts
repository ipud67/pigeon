...
// projection of one. Schema follows the brief + spec §6.5 + the v2 three-layer model
...
// clearly-labeled placeholder. A hand-researched data/depth-overrides.json entry (Research)
...
// ---- FULL EVENTS (product ruling 2026-07-02) ------------------------------
// A FULL EVENT is a source item that IS the complete event artifact — a whole press
// briefing, press conference, transcript, hearing, or oral argument, start to finish.
// Tagged by precision-first URL/keyword heuristics at ingest (lib/editorial/fullevent.ts)
// and backfilled at read time for the committed dataset. Surfaced in BOTH places: a labeled
// linkout block inside the story long form, and the standalone /events/ browsing page.
// LINKOUT ONLY — never embedded or hosted ( stands; embeds are Godfather-gated).

export type FullEventKind =
 | 'press_briefing'
 | 'press_conference'
 | 'remarks'
 | 'transcript'
...
export type FullEvent = {
 kind: FullEventKind;
 label: string; // plain reader label, e.g. "Press briefing" / "Parliament debate — full record"
 url: string; // linkout to the complete artifact
 outlet: string; // source name
 paywalled?: boolean; // red line: a paywalled full source is always flagged
};
...
export type FactRecord = {
 ...
 // DEPTH (spec §B): every headline IS its long form. Constitutional analysis is
 ...
 depth?: FactDepth;
 // IMPORTANCE (spec §A): 0-100 score + tier set by the ranking engine at ingest. The
 // home/week feeds rank by this and bury micro-noise (8-K etc.).
 importance?: number; // 0-100
 // LEAD added 2026-07-22 — the front page assigns it to the top of the ranked field.
 importance_tier?: 'LEAD' | 'HIGH' | 'MED' | 'LOW' | 'BURIED';
 longform_url?: string; // optional full press-event / stream linkout
 // FULL EVENT : set when the source item IS the complete event artifact. Precision
 // over recall — a mislabeled full event is worse than a missed one.
