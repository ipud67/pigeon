...
import { deriveFullEvent } from './editorial/fullevent';
...
export function readFacts(): FactRecord[] {
 ...
 // FULL EVENT backfill : deterministic tagging applied at read time so the committed
 // dataset gets tagged WITHOUT a re-ingest (a fresh feed pull would rotate fact ids and
 // orphan Research's depth overrides). New ingests also bake the field in at build.
 return facts
 .slice()
 .sort((a, b) => (a.datetime_utc < b.datetime_utc ? 1 : -1))
 .map((f) => (f.full_event ? f : { ...f, full_event: deriveFullEvent(f) }));
}
