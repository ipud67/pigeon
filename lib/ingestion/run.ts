...
//   5. generate rule-based WEIGH-IT questions — no LLM
...
import { semanticMerge } from './semantic-cluster';
...
async function main() {
  ...
  // 3 — dedup / cluster: lexical pass (exact URL + near-match title), then a semantic pass
  // (embeddings, PHASE 1 of docs/SEMANTIC_RANKING_SPEC_2026-07-23.md) that catches the same
  // action described in headlines sharing zero words. $0, local, CPU-only — never blocks the
  // run if the model can't load (offline dev box, first-download blocked).
  const lexicalClusters = clusterItems(admitted);
  const semantic = await semanticMerge(lexicalClusters);
  const clusters = semantic.clusters;
  if (semantic.skippedReason) {
    console.log(`\n  ⚠ semantic clustering skipped (${semantic.skippedReason}) — lexical dedup only`);
  } else {
    console.log(
      `\n  semantic clustering: ${lexicalClusters.length} clusters -> ${clusters.length} ` +
        `(${semantic.merges.length} merge${semantic.merges.length === 1 ? '' : 's'}, ` +
        `embedded ${semantic.itemsEmbedded} in ${semantic.embedMs}ms)`,
    );
    for (const m of semantic.merges) {
      console.log(`    merged (sim ${m.similarity.toFixed(2)}): "${m.a.slice(0, 55)}" + "${m.b.slice(0, 55)}"`);
    }
  }
